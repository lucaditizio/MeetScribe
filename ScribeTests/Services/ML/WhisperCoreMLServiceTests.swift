...
final class WhisperCoreMLServiceTests: XCTestCase {
    private var service: WhisperCoreMLService!
    private var config: PipelineConfig!
    
    override func setUp() {
        super.setUp()
        config = PipelineConfig()
        service = WhisperCoreMLService(config: config)
    }
    ...
    func testEmptyAudioThrowsError() async {
        ...
        do {
            _ = try await service.transcribe(audioData: emptyData, language: "gsw")
    ...
    func testInsufficientSamplesThrowsError() async {
        ...
        do {
            _ = try await service.transcribe(audioData: insufficientData, language: "gsw")
            XCTFail("Expected error to be thrown")
        } catch {
            XCTAssertTrue(true, "Error thrown as expected")
        }
    ...
    func testInvalidAudioFormatThrowsError() async {
        ...
        do {
            _ = try await service.transcribe(audioData: invalidData, language: "gsw")
            XCTFail("Expected error to be thrown")
        } catch {
            XCTAssertTrue(true, "Error thrown as expected")
        }
    ...
    func testValidAudioDataTriesToLoadModel() async {
        ...
        do {
            _ = try await service.transcribe(audioData: validData, language: "gsw")
